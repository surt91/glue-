...
template<class T>
Histogram histogramFromStream(T &instream, int num_bins, double lower, double upper, int column=0)
{
...
template<class T>
void bordersFromStream(T &instream, double &lower, double &upper, int column=0)
{
    while(instream.good())
    {
        std::string line = getNextLine(instream);
